...
if ($ortBinRoot -and (Test-Path $ortBinRoot)) {
    Copy-Item -Path (Join-Path $ortBinRoot "onnxruntime.dll") -Destination $artifactRoot -Force -ErrorAction SilentlyContinue
    Copy-Item -Path (Join-Path $ortBinRoot "onnxruntime_providers_shared.dll") -Destination $artifactRoot -Force -ErrorAction SilentlyContinue
}
